...
- **(2026-06-28) Corrección Bug Botón = (Color Picker):** Se implementó correctamente la función matemática `mixHexWithWhite` para que el selector de color del botón `=` muestre el tono exacto (mezclado con blanco) sin generar un error silencioso en Javascript que bloqueaba el evento click en el Modo Diseño.
- **(2026-06-28) Corrección Botón C en Herramientas:** Se actualizó el comportamiento del botón `C` cuando se está utilizando una herramienta (Aumento, Descuento, IVA). Ahora, al pulsarlo, no solo limpia la caja de texto activa, sino que también pone a cero la pantalla principal de la calculadora y borra el mensaje explicativo del último cálculo.
- **(2026-06-28) Refinamiento Texto Modo Diseño:** Se reemplazó el texto de ejemplo `(ej. "Números")` del mensaje de alerta inicial del Modo Diseño por la instrucción más precisa "debajo de EDITANDO:".
- **(2026-06-28) Corrección Persistencia de Temas (v59):** Se corrigió un problema donde, al guardar o renombrar un tema, este no se establecía inmediatamente como el tema activo interno, provocando que al reiniciar la app no cargara el diseño recién guardado.
- **(2026-06-28) Sobreescritura de Temas de Fábrica (v60):** Se añadió la posibilidad de editar colores y cambiar el nombre directamente a los 20 temas de fábrica sin necesidad de crear una copia obligatoria. Los cambios en los temas por defecto se guardan en el almacenamiento local y se aplican al reiniciar.
- **(2026-06-28) Etiquetas Flotantes en Temas de Fábrica (v61):** Se implementó el mismo sistema de etiquetas flotantes (tooltips) al mantener pulsado ("long press") para los temas de fábrica que ya existía para los temas personalizados.
